    ...
    <main className="px-4 pt-5">
      ...
      {feed.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="text-sm font-semibold text-mid">No notifications yet</p>
          <p className="mt-1 text-xs text-dim">Booking updates and messages will appear here.</p>
        </div>
      ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-bold">
                        {cat ? `${cat.icon} ${cat.label}` : "Booking"}
                        {b ? ` · ${b.workerName.split(" ")[0]}` : ""}
                      </p>
                      <span className="shrink-0 text-[10px] text-dim">{timeAgo(m.createdAt)}</span>
                    </div>
                    <p className="mt-0.5 line-clamp-2 text-xs text-mid">
                      {m.text ?? (m.kind === "image" ? "📷 Photo" : m.kind === "video" ? "🎥 Video" : "")}
                    </p>
                  </div>
                </Card>
              </Link>
